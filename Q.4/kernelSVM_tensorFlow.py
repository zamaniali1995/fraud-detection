...
b = tf.Variable(tf.random_normal(shape=[1, batch_size]), name="b")
...
prediction_grid = tf.placeholder(shape=[None, 30], dtype=tf.float32, name= "grid")
...
with tf.Session() as sess:
    ...
    for epoch in range(epoch_num): 
        ...
        train_acc_list.append(sess.run(
                accuracy, feed_dict={x: train_X, y: 
                    train_y, prediction_grid: train_X})*100)
        test_acc_list.append(sess.run(accuracy
                                      , feed_dict={x: test_X,
                                                   y:  test_y, prediction_grid:test_X})*100)
    ...
    train_pridected = sess.run(prediction_output, feed_dict = {x: train_X})
#    w_value, b_value = sess.run([w, b])
    test_pridected = sess.run(prediction_output, feed_dict = {x:test_X})
##%%1
